...
- `/healthz` (liveness, plus the deployed commit from `RENDER_GIT_COMMIT`, so a healthy-but-stale deployment is machine-detectable; the same short SHA sits in every screen's footer) and `/readyz` (database connectivity + morpholog binary + a commit-layer round-trip + the projector's **progress** where it runs in-process). The Overview screen's health tile renders the same call, so the probe and the screen cannot disagree.
