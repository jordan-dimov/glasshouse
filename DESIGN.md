...
Because both read-side categories are mechanically checkable, the law yields a product pillar, not just a discipline: **`glasshouse verify`** replays the projections and compares checksums, re-hashes the payloads against their admitted hashes, checks the deployed model hash against what the ledger was committed under, verifies evidence-pack manifests, and reports orphans, missing payloads and (once Morpholog's hash-chained audit log lands) broken transition chains. The promise it makes is concrete and rare: *you can prove, on demand, that the operational database still agrees with the governed ledger.* No ETRM ships that; here it falls out of one sentence of discipline, and it sits alongside import, P&L, corrections and audit as a headline capability.
...
Glasshouse depends on Morpholog for governed commits, and needs five integration surfaces, all agreed with upstream (06/06/2026): **batch proposal execution** (`run --batch` -- accepted; each row its own transition, never all-or-nothing), a **canonical model hash** (accepted; rules-identity over canonical source), a **schema manifest** (accepted; one artefact feeds all codegen and CI drift-checks), **generated per-predicate SQL views** as the inspection surface (upstream's improvement on our filtering ask), and eventually **pattern-based authority** (parked; our capability model is the forcing example). Evidence-pack manifest extension points and the hash-chained audit log are coordinated; PG18 is in upstream CI. **v0 blocks on none of this** -- it builds against current Morpholog and adopts each surface as it lands.
...
- **Generated, typed, checked — not magically dynamic**: Pydantic request models are generated from the commit layer's own schemas (`morpholog schema`) in a build step, committed to the repo, and CI fails on drift. The API, its validation, its OpenAPI docs and the governed rule contract come from one source; runtime-dynamic generation is deferred until ergonomics clearly demand it.
- One tagged-value codec (a Pydantic discriminated union) handles the wire format both directions, including outbox intent payloads. Money and quantities are Decimal from strings, never via float.
- Typed outcomes: the adapter returns `Committed | Rejected` as a discriminated union and raises on operational failure — the two most likely integration bugs (rejection-vs-failure, gate-vs-question) made unrepresentable.

## 8. Stack

Python **3.13+** (CI matrix 3.13 and 3.14; the floor is adoption-friendliness, the ceiling is dogfooding) · uv (locked) · ruff · mypy (blocking) · pytest + Hypothesis · FastAPI + Pydantic v2 · SQLAlchemy 2 Core (governed state read only via the generated per-predicate views; full on app schema) + psycopg3 · Alembic (app schema only) · structlog (JSON, run-id via contextvars) · **PostgreSQL 17+ floor (matching Morpholog), demo and CI lead on 18** -- 18 is preferred, not required: its `uuidv7()`, async I/O and skip-scan improvements are nice; Glasshouse mints UUIDv7 app-side so no PG18-only feature is load-bearing, and raising the floor above the substrate's would cost adoption for nothing. + **TimescaleDB (a dependency, not an option)**: curve points as hypertables, compression for superseded curve versions (the supersede-never-overwrite discipline creates exactly the storage growth compression solves), continuous aggregates for period-to-day rollups. One code path, one set of migrations; the storage *law* (projection-or-payload) is independent of Timescale -- it is how the law is implemented efficiently, which is exactly why a single code path beats a dual "profile" nobody tests. Render supports the extension (verify `CREATE EXTENSION timescaledb` on Render PG18 at scaffold time, before anything depends on it -- extension availability varies by PG version per Render's docs); self-hosters use the timescaledb image or install one extension. Boundary agreed with upstream: Timescale sits **beside Morpholog, never under it** -- hypertables live in the app schema only; the morpholog schema stays plain PG primitives (its replay correctness rides on SSI + transition ordering, exactly the interaction chunking must not touch). One operational coupling to note: a shared instance means a shared extension-upgrade cadence; if BESS telemetry volume grows serious, a separate Timescale instance fed by the outbox keeps the governed core's operational profile boring · the `morpholog` binary via multi-stage Docker (Rust builder stage; `MORPHOLOG_BIN` for dev) · outbox worker + projector as a second process · docker-compose for the full stack · GitHub Actions running format, lint, types, tests, plus the schema-drift contract job.
...
- Postgres 18 satisfies Morpholog's stated 17+ requirement; smoke-test against 18 once at setup (open item).
...
- Smoke-test `CREATE EXTENSION timescaledb` on Render PG18 at scaffold time, before anything depends on it (extension availability varies by PG version per Render's docs).
