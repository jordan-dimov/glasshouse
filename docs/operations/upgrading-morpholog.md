...
A change to a predicate's shape is not a migration: morpholog has no claim migration, so rows admitted under the old declaration are skew to the new binary (the named decode refuses them, the arity guard counts them). Every Glasshouse database today is fresh-`init` and the demo's nightly reset re-provisions, so a reshape costs nothing yet, but the usual order above is wrong for it: web's pre-deploy `provision` would run the app-schema migration (wiping the projections) while the running web instance's projector meets old-shaped rows and refuses (`projector` red on `/readyz` inside about fifteen seconds).

**Order for a model change** (first exercised by the trade amendment, contract section 26): suspend the cron, deploy the cron, run the one-off `seed --reset` (the governed schema is dropped and re-initialised at the new shape, then seeded and verified), then deploy web. Declare the window; there is no zero-downtime path across a reshape until upstream has a claim migration story.
